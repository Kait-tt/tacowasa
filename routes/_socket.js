...
// const stages = require('../lib/model/stages');
// const LoggerSocket = require('../lib/model/loggerSocket');
// const GitHub = require('../lib/model/github');
// const queue = require('../lib/module/asyncQueue');
// const ChatLog = require('../lib/model/chatLog');


let io;
let emitters;
const firstSendChatLimit = 200;

function socketRouting(server) {
    io = socketio.listen(server);
    // TODO: logger
    // const loggerIO = new LoggerSocket.IO(io);
    // module.exports.io = io = loggerIO.io;
...
// method(req, params) return promise
module.exports.emitters = emitters = {
    joinRoom: (req, {projectId, username}) => {
        io.to(projectId).emit('join-room', {username: username});
        notifyText(projectId, username, 'joined room');
        return Promise.resolve({req, params: {projectId, username}});
    },

    leaveRoom: (req, {projectId, username}) => {
        io.to(projectId).emit('leave-room', {username: username});
        notifyText(projectId, username, 'left room');
        return Promise.resolve({req, params: {projectId, username}});
    },

    removeMember: ({projectId, username, token}={}, targetUserName, fn) => {
    ...
    addMember: ({projectId, username, token}={}, targetUserName, fn) => {
    ...
    updateMember: ({projectId, username, token}={}, targetUserName, updateParams, fn) => {
        Project.updateMember({id: projectId}, targetUserName, updateParams, function (err, project, member) {
            ...
            io.to(projectId).emit('update-member', {member: member});
            notifyText(projectId, username, 'updated member: "' + targetUserName + '" , ' + JSON.stringify(updateParams));
        });
    },

    updateMemberOrder: ({projectId, username, token}={}, userName, insertBeforeOfUserName, fn) => {
        Project.updateMemberOrder({id: projectId}, userName, insertBeforeOfUserName, function (err, project, member, insertBeforeOfMember) {
           ...
           xports.io.to(projectId).emit('update-member-order', {member: member, userName: userName,
    ...
    addIssue: ({projectId, username, token}={}, params, fn) => {
    ...
    removeIssue: ({projectId, username, token}={}, issueId, fn) => {
    ...
    // ステージとアサインを同時に処理する
    updateStage: ({projectId, username, token}={}, issueId, toStage, userId, fn) => {
        Project.updateStage({id: projectId}, token, issueId, toStage, userId, function (err, project, issue) {
            ...
            io.to(projectId).emit('update-stage', {issue: issue, issueId: issueId, toStage: toStage, assignee: userId});
    ...
    updateIssueDetail: ({projectId, username, token}={}, issueId, title, body, cost, fn) => {
    ...
    updateIssueWorkingState: ({projectId, username, token}={}, issueId, isWorking, fn) => {
        queue.push(projectId, function (done) {
            Project.updateIssueWorkingState({id: projectId}, issueId, isWorking, function (err, project, issue) {
                ...
                io.to(projectId).emit('update-issue-working-state', { issue: issue, issueId: issueId, isWorking: isWorking });
                notifyText(projectId, username, (isWorking ? 'start to work: ' : 'stop to work: ') + issue.title);
            });
        });
    },

    updateIssueWorkHistory: ({projectId, username, token}={}, issueId, workHistory, fn) => {
        queue.push(projectId, function (done) {
            Project.updateIssueWorkHistory({id: projectId}, issueId, workHistory, function (err, project, issue) {
                ...
                io.to(projectId).emit('update-issue-work-history', {issue: issue, workHistory: issue.workHistory});
                notifyText(projectId, username, 'updated work history: ' + issue.title);
            });
        });
    },

    updateIssuePriority: ({projectId, username, token}={}, issueId, insertBeforeOfIssueId, fn) => {
        Project.updateIssuePriority({id: projectId}, issueId, insertBeforeOfIssueId, function (err, project, issue, insertBeforeOfIssueId) {
            ...
            io.to(projectId).emit('update-issue-priority', {issue: issue, issueId: issueId, insertBeforeOfIssueId: insertBeforeOfIssueId});
    ...
    attachLabel: ({projectId, username, token}={}, issueId, labelName, fn) => {
        queue.push(projectId, function (done) {
            Project.attachLabel({id: projectId}, token, issueId, labelName, function (err, project, issue, label) {
                ...
                if (issue && label) {
                    io.to(projectId).emit('attach-label', { issue: issue, issueId: issueId, label: label });
    ...
    detachLabel: ({projectId, username, token}={}, issueId, labelName, fn) => {
        queue.push(projectId, function (done) {
            Project.detachLabel({id: projectId}, token, issueId, labelName, function (err, project, issue, label) {
                ...
                if (issue && label) {
                    io.to(projectId).emit('detach-label', { issue: issue, issueId: issueId, label: label });
    ...
    syncLabelAll: ({projectId, username, token}={}, fn) => {
        queue.push(projectId, function (done) {
            Project.findOne({id: projectId}, function (err, project) {
                ...
                github.syncLabelsFromGitHub(project.github.repoName, project.github.userName, project, function (err, project) {
                    ...
                    github.syncIssuesFromGitHub(project.github.repoName, project.github.userName, project, ['labels'], function (err, project) {
                        ...
                        io.to(projectId).emit('sync-label-all', {project: project});
