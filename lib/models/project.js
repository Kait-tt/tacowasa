...
class Project {
    static get defaultFindOption() {
        return {
            include: [
                ...
                {model: db.Task, as: 'tasks', include: [
    ...
    static create(name, username) {
        return co(function* () {
            ...
            // set default options
            const defaultAccessLevel = yield db.AccessLevel.findOne({where: {projectId: project.id, name: 'Developer'}});
            const defaultStage = yield db.Stage.findOne({where: {projectId: project.id, name: 'issue'}});
            const defaultCost = yield db.Cost.findOne({where: {projectId: project.id, name: 'undecided'}});
            ...
            // add owner as member
            const ownerAccessLevel = yield db.AccessLevel.findOne({where: {projectId: project.id, name: 'Owner'}});
            yield Member.add(project.id, username, {
                wipLimit: project.defaultWipLimit,
                accessLevelId: ownerAccessLevel.id
            });

            return yield Project.findOneIncludeAll({where: {id: project.id}});
        });
    }
    ...
    static createDefaultCosts(projectId, options={}) {
        return db.Cost.bulkCreate([
            {projectId, name: 'undecided',   value: 99},
