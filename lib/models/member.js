...
class Member {
    ...
    static updateOrder (projectId, username, beforeUsername, {transaction} = {}) {
        return db.sequelize.transaction({transaction}, transaction => {
            return co(function* () {
                ...
                // update old around links
                {
                    const {prevMemberId, nextMemberId} = user.member;
                    if (prevMemberId) {
                        yield db.Member.update({nextMemberId}, {where: {projectId, id: prevMemberId}, transaction});
                    }
                    if (nextMemberId) {
                        yield db.Member.update({prevMemberId}, {where: {projectId, id: nextMemberId}, transaction});
                    }
                }
                ...
                // update prev member
                if (prevMemberId) {
                    yield db.Member.update({nextMemberId: user.member.id}, {where: {projectId, id: prevMemberId}, transaction});
                }
                // update next member
                if (nextMemberId) {
                    yield db.Member.update({prevMemberId: user.member.id}, {where: {projectId, id: nextMemberId}, transaction});
                }
                // update target member
                yield db.Member.update({prevMemberId, nextMemberId}, {where: {projectId, id: user.member.id}, transaction});

                return {
                    user: yield Member.findByUserId(projectId, user.id),
                    beforeUser: beforeUser ? (yield Member.findByUserId(projectId, beforeUser.id)) : null
                };
            });
        });
    }
