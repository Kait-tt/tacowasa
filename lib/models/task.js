    ...
    static _validateStageAndUser (projectId, stageId, userId, task, {transaction} = {}) {
        return co(function* () {
            ...
            if (stage.assigned && !userId) {
                throw new Error(`no assignment is invalid with the stage(${stageId}) in ${projectId}. (${JSON.stringify(task)})`);
            }
            if (!stage.assigned && userId) {
                throw new Error(`assignment is invalid with the stage(${stageId} in ${projectId}. (${JSON.stringify(task)})`);
            }
        });
    }
