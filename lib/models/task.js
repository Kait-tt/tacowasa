...
class Task {
    ...
    static create (projectId, {title, body, stageId, userId, costId, labelIds = []}, {transaction} = {}) {
        return db.sequelize.transaction({transaction}, transaction => {
            return co(function* () {
                ...
                yield Task.updateCompletedAt(newTask.id, {transaction});
    ...
    static updateContent (projectId, taskId, {title, body, costId}, {transaction} = {}) {
        return db.coTransaction({transaction}, function* (transaction) {
            ...
            yield db.Task.update(updateParams, {where: {projectId, id: taskId}, transaction});
    ...
    static updateStatus (projectId, taskId, {userId = null, stageId}, {transaction} = {}) {
        return db.sequelize.transaction({transaction}, transaction => {
            return co(function* () {
                ...
                yield Task.updateCompletedAt(taskId, {transaction});
    ...
    // start or stop work
    static updateWorkingState (projectId, taskId, isWorking, {transaction} = {}) {
        return db.coTransaction({transaction}, function* (transaction) {
            ...
            yield db.Task.update({isWorking}, {where: {projectId, id: taskId}, transaction});

            if (isWorking) { // start
                yield db.Work.create({userId: task.user.id, taskId: task.id, stageId: task.stage.id}, {transaction});
            } else { // stop
                const lastWork = _.find(task.works, {isEnded: false});
                if (!lastWork) {
                    throw new Error(`work was not found of ${taskId} in ${projectId}.`);
                }
                yield db.Work.update({isEnded: true, endTime: Date.now()}, {where: {id: lastWork.id}, transaction});
            }
            return yield Task.findById(taskId, {transaction});
        });
    }
    ...
    static updateWorkHistory (projectId, taskId, works, {transaction} = {}) {
        return co(function* () {
            // validation
            ...
            const existsWorks = yield db.Work.findAll({where: {taskId}, transaction});

            // remove all
            for (let work of existsWorks) {
                yield work.destroy({where: {}, transaction});
            }

            // add all
            const _works = works.map(x => _.defaults({taskId}, x));
            yield db.Work.bulkCreate(_works, {transaction});
            return yield Task.findById(taskId, {transaction});
        });
    }
            ...
            return co(function* () {
                ...
                if (taskId === beforeTaskId) {
                    return {
                        ...
                        beforeTask: yield Task.findOne({where: {id: beforeTaskId, projectId}, transaction}),
                        updated: false
                    };
                }
    ...
    static updateCompletedAt (taskId, {transaction} = {}) {
        return db.coTransaction({transaction}, function* (transaction) {
            const task = yield db.Task.findOne({where: {id: taskId}, transaction});
            const stage = yield db.Stage.findOne({where: {id: task.stageId}, transaction});
    ...
    static validateTaskOrder (projectId, {transaction} = {}) {
        return db.coTransaction({transaction}, function* (transaction) {
