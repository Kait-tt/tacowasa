...
class Task {
    ...
    static validateTaskOrder (projectId, {transaction} = {}) {
        return db.coTransaction({transaction}, function* (transaction) {
            ...
            tasks.forEach(x => {
               if (x.nextTaskId) {
                   ...
                   if (!nx || nx.prevTaskId !== x.id) {
                       throw new Error(`x.nextTask.prevTask != x : ${x.id}`);
                   }
               }
               ...
               if (x.prevTaskId) {
                   ...
                   if (!nx || nx.nextTaskId !== x.id) {
                       throw new Error(`x.prevTask.nextTask != x : ${x.id}`);
                   }
               }
            });
            ...
            if (!firsts.length) {
                throw new Error('first task was not found');
            }
            if (firsts.length > 1) {
                throw new Error('first tasks are found over 2');
            }
            if (!lasts.length) {
                throw new Error('last task was not found');
            }
            if (lasts.length > 1) {
                throw new Error('last tasks are found over 2');
            }

            // all connected ?
            let current = firsts[0];
            const used = {};
            used[current.id] = true;
            while (current.nextTaskId) {
                current = id2Task[current.nextTaskId];
                if (used[current.id]) {
                    throw new Error(`loop with ${current.id}`)
                }
                used[current.id] = true;
            }
        });
