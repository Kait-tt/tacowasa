...
class Task {
    ...
    static async updateStatus (projectId, taskId, {userId = null, stageId}, {transaction} = {}) {
        return db.transaction({transaction}, async transaction => {
            ...
            const beforeStage = await db.Stage.findById(task.stageId, {transaction});

            await db.Task.update({userId, stageId}, {where: {projectId, id: taskId}, transaction});

            const afterStage = (await db.Task.findById(taskId, {
                include: [ {model: db.Stage, as: 'stage'} ],
                transaction
            })).stage;

            if (!['done', 'archive'].includes(beforeStage.name) && !['done', 'archive'].includes(afterStage.name)) {
