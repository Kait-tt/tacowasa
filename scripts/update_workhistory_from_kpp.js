...
async function updateWorkHistory ({transaction} = {}) {
    ...
    for (let {id, issues} of srcProjects) {
        ...
        for (let {title, workHistory: srcWorks, github: {number: githubNumber}} of issues) {
            ...
            const githubTask = await db.GitHubTask.findOne({where: {projectId: project.id, number: String(githubNumber)}, transaction});
            const task = await db.Task.findOne({where: {id: githubTask.taskId}, transaction});

            const oldWorks = await db.Work.findAll({where: {taskId: task.id}, transaction});
            ...
            await Task.updateWorkHistory(project.id, task.id, works, {transaction});
        }
    }
}
...
function main () {
    db.transaction(async transaction => {
        ...
        await updateWorkHistory({transaction});
        console.log('done');
